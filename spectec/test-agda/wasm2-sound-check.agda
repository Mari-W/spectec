...
data Limits-sub : limits → limits → Set where
  max : ∀ (n-1 : n) (m-1 : m) (n-2 : n) (m-2-opt : (Maybe m)) → 
    (n-1 ≥ n-2) →
    Forall (λ (m-2 : ℕ) → (m-1 ≤ m-2)) (fromMaybe m-2-opt) →
    Limits-sub (mk-limits (mk-uN n-1) (just (mk-uN m-1))) (mk-limits (mk-uN n-2) (mapMaybe (λ (m-2 : ℕ) → (mk-uN m-2)) m-2-opt))
  eps : ∀ (n-1 : n) (n-2 : n) → 
    (n-1 ≥ n-2) →
    Limits-sub (mk-limits (mk-uN n-1) nothing) (mk-limits (mk-uN n-2) nothing)
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:533.1-533.78 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:534.1-534.77 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:535.1-535.78 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:570.1-570.75 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:572.1-572.72 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:576.1-576.77 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:574.1-574.73 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:577.1-577.74 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:643.1-643.83 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:642.1-642.80 -}
...
{- Inductive Relations Definition at: ../specification/wasm-2.0/6-typing.spectec:673.1-673.62 -}
...
data Context-ok : context → Set where
  mk-Context-ok : ∀ (C : context) (ft-lst : (List functype)) (ft-2-lst : (List functype)) (gt-lst : (List globaltype)) (tt-lst : (List tabletype)) (mt-lst : (List memtype)) (et-lst : (List elemtype)) (ok-lst : (List datatype)) (lct-lst : (List valtype)) (rt-lst : (List reftype)) (rt'-opt : (Maybe reftype)) → 
    (C ≡ record { context-TYPES = ft-lst ; context-FUNCS = ft-2-lst ; context-GLOBALS = gt-lst ; context-TABLES = tt-lst ; context-MEMS = mt-lst ; context-ELEMS = et-lst ; context-DATAS = ok-lst ; context-LOCALS = lct-lst ; LABELS = (as (List resulttype) ((mk-list (map (λ (rt : reftype) → (valtype-reftype rt)) rt-lst)) ∷ [])) ; context-RETURN = (just (mk-list (fromMaybe (mapMaybe (λ (rt' : reftype) → (valtype-reftype rt')) rt'-opt)))) }) →
    Forall (λ (ft : functype) → (Functype-ok ft)) ft-lst →
    Forall (λ (gt : globaltype) → (Globaltype-ok gt)) gt-lst →
    Forall (λ (mt : memtype) → (Memtype-ok mt)) mt-lst →
    Forall (λ (tt' : tabletype) → (Tabletype-ok tt')) tt-lst →
    Forall (λ (ft-2 : functype) → (Functype-ok ft-2)) ft-2-lst →
    Context-ok C
...
data Ref-ok : store → ref → reftype → Set where
  null : ∀ (s : store) (rt : reftype) → Ref-ok s (ref-REF-NULL rt) rt
  Ref-ok--func : ∀ (s : store) (a : addr) (ext : functype) → 
    (Externaddr-ok s (externaddr-FUNC a) (FUNC ext)) →
    Ref-ok s (REF-FUNC-ADDR a) FUNCREF
  extern : ∀ (s : store) (a : addr) → Ref-ok s (REF-HOST-ADDR a) EXTERNREF
...
data Val-ok : store → val → valtype → Set where
  Val-ok--numtype : ∀ (s : store) (nt : numtype) (c-t : (num- nt)) → Val-ok s (val-CONST nt c-t) (valtype-numtype nt)
  Val-ok--vectype : ∀ (s : store) (vt : vectype) (c-t : (uN-fam0 ((unwrap! (size (valtype-vectype vt)))))) → Val-ok s (val-VCONST vt c-t) (valtype-vectype vt)
  Val-ok--reftype : ∀ (s : store) (r : ref) (rt : reftype) → 
    (Ref-ok s r rt) →
    Val-ok s (val-ref r) (valtype-reftype rt)

{- Inductive Relations Definition at: ../specification/wasm-2.0/B-soundness.spectec:55.1-55.47 -}
data Result-ok : store → result → (List valtype) → Set where
  Result-ok--result : ∀ (s : store) (v-lst : (List val)) (t-lst : (List valtype)) → 
    ((length t-lst) ≡ (length v-lst)) →
    Forall₂ (λ (t : valtype) (v : val) → (Val-ok s v t)) t-lst v-lst →
    Result-ok s (-VALS v-lst) t-lst
  trap : ∀ (s : store) (t-lst : (List valtype)) → Result-ok s TRAP t-lst

{- Type Alias Definition at: ../specification/wasm-2.0/B-soundness.spectec:66.1-66.31 -}
adminexpr : Set
adminexpr = (List admininstr)
...
data Moduleinst-ok : store → moduleinst → context → Set where
  mk-Moduleinst-ok : ∀ (s : store) (functype-lst : (List functype)) (funcaddr-lst : (List funcaddr)) (globaladdr-lst : (List globaladdr)) (tableaddr-lst : (List tableaddr)) (memaddr-lst : (List memaddr)) (elemaddr-lst : (List elemaddr)) (dataaddr-lst : (List dataaddr)) (exportinst-lst : (List exportinst)) (functype-F-lst : (List functype)) (globaltype-lst : (List globaltype)) (tabletype-lst : (List tabletype)) (memtype-lst : (List memtype)) (elemtype-lst : (List elemtype)) (datatype-lst : (List datatype)) → 
    Forall (λ (v-functype : functype) → (Functype-ok v-functype)) functype-lst →
    ((length globaladdr-lst) ≡ (length globaltype-lst)) →
    Forall₂ (λ (v-globaladdr : globaladdr) (v-globaltype : globaltype) → (Externaddr-ok s (externaddr-GLOBAL v-globaladdr) (GLOBAL v-globaltype))) globaladdr-lst globaltype-lst →
    ((length funcaddr-lst) ≡ (length functype-F-lst)) →
    Forall₂ (λ (v-funcaddr : funcaddr) (functype-F : functype) → (Externaddr-ok s (externaddr-FUNC v-funcaddr) (FUNC functype-F))) funcaddr-lst functype-F-lst →
    ((length memaddr-lst) ≡ (length memtype-lst)) →
    Forall₂ (λ (v-memaddr : memaddr) (v-memtype : memtype) → (Externaddr-ok s (externaddr-MEM v-memaddr) (MEM v-memtype))) memaddr-lst memtype-lst →
    ((length tableaddr-lst) ≡ (length tabletype-lst)) →
    Forall₂ (λ (v-tableaddr : tableaddr) (v-tabletype : tabletype) → (Externaddr-ok s (externaddr-TABLE v-tableaddr) (TABLE v-tabletype))) tableaddr-lst tabletype-lst →
    ...
    ((length ((map (λ (v-globaladdr : globaladdr) → (externaddr-GLOBAL v-globaladdr)) globaladdr-lst) ++ ((map (λ (v-memaddr : memaddr) → (externaddr-MEM v-memaddr)) memaddr-lst) ++ ((map (λ (v-tableaddr : tableaddr) → (externaddr-TABLE v-tableaddr)) tableaddr-lst) ++ (map (λ (v-funcaddr : funcaddr) → (externaddr-FUNC v-funcaddr)) funcaddr-lst))))) > 0) →
    Forall (λ (v-exportinst : exportinst) → ((ADDR v-exportinst) ∈ ((map (λ (v-globaladdr : globaladdr) → (externaddr-GLOBAL v-globaladdr)) globaladdr-lst) ++ ((map (λ (v-memaddr : memaddr) → (externaddr-MEM v-memaddr)) memaddr-lst) ++ ((map (λ (v-tableaddr : tableaddr) → (externaddr-TABLE v-tableaddr)) tableaddr-lst) ++ (map (λ (v-funcaddr : funcaddr) → (externaddr-FUNC v-funcaddr)) funcaddr-lst)))))) exportinst-lst →
    Moduleinst-ok s record { TYPES = functype-lst ; FUNCS = funcaddr-lst ; GLOBALS = globaladdr-lst ; TABLES = tableaddr-lst ; MEMS = memaddr-lst ; ELEMS = elemaddr-lst ; DATAS = dataaddr-lst ; EXPORTS = exportinst-lst } record { context-TYPES = functype-lst ; context-FUNCS = functype-F-lst ; context-GLOBALS = globaltype-lst ; context-TABLES = tabletype-lst ; context-MEMS = memtype-lst ; context-ELEMS = elemtype-lst ; context-DATAS = datatype-lst ; context-LOCALS = [] ; LABELS = [] ; context-RETURN = nothing }
...
data Frame-ok : store → frame → context → Set where
  mk-Frame-ok : ∀ (s : store) (val-lst : (List val)) (v-moduleinst : moduleinst) (C : context) (t-lst : (List valtype)) → 
    (Moduleinst-ok s v-moduleinst C) →
    ((length t-lst) ≡ (length val-lst)) →
    Forall₂ (λ (t : valtype) (v-val : val) → (Val-ok s v-val t)) t-lst val-lst →
    Frame-ok s record { LOCALS = val-lst ; frame-MODULE = v-moduleinst } (C ⧺ record { context-TYPES = [] ; context-FUNCS = [] ; context-GLOBALS = [] ; context-TABLES = [] ; context-MEMS = [] ; context-ELEMS = [] ; context-DATAS = [] ; context-LOCALS = t-lst ; LABELS = [] ; context-RETURN = nothing })

mutual
  {- Inductive Relations Definition at: ../specification/wasm-2.0/B-soundness.spectec:68.1-69.36 -}
  data Instr-ok2 : store → context → admininstr → functype → Set where
    plain : ∀ (s : store) (C : context) (v-instr : instr) (t-1-lst : (List valtype)) (t-2-lst : (List valtype)) → 
      (Instr-ok C v-instr (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))) →
      Instr-ok2 s C (admininstr-instr v-instr) (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))
    label : ∀ (s : store) (C : context) (v-n : n) (instr'-lst : (List instr)) (admininstr-lst : (List admininstr)) (t-lst : (List valtype)) (t'-lst : (List valtype)) → 
      ((length (t'-lst)) ≡ (v-n)) →
      (Instrs-ok2 s C (map (λ (instr' : instr) → (admininstr-instr instr')) instr'-lst) (mk-functype (mk-list t'-lst) (mk-list t-lst))) →
      (Instrs-ok2 s (record { context-TYPES = [] ; context-FUNCS = [] ; context-GLOBALS = [] ; context-TABLES = [] ; context-MEMS = [] ; context-ELEMS = [] ; context-DATAS = [] ; context-LOCALS = [] ; LABELS = (as (List resulttype) ((mk-list t'-lst) ∷ [])) ; context-RETURN = nothing } ⧺ C) admininstr-lst (mk-functype (mk-list []) (mk-list t-lst))) →
      Instr-ok2 s C (LABEL- v-n instr'-lst admininstr-lst) (mk-functype (mk-list []) (mk-list t-lst))
    Instr-ok2--frame : ∀ (s : store) (C : context) (v-n : n) (f : frame) (admininstr-lst : (List admininstr)) (t-lst : (List valtype)) (C' : context) → 
      ((length (t-lst)) ≡ (v-n)) →
      (Frame-ok s f C') →
      (Expr-ok2 s (record { context-TYPES = [] ; context-FUNCS = [] ; context-GLOBALS = [] ; context-TABLES = [] ; context-MEMS = [] ; context-ELEMS = [] ; context-DATAS = [] ; context-LOCALS = [] ; LABELS = [] ; context-RETURN = (just (mk-list t-lst)) } ⧺ C') admininstr-lst (mk-list t-lst)) →
      Instr-ok2 s C (FRAME- v-n f admininstr-lst) (mk-functype (mk-list []) (mk-list t-lst))
    Instr-ok2--call-addr : ∀ (s : store) (C : context) (v-funcaddr : funcaddr) (t-1-lst : (List valtype)) (t-2-lst : (List valtype)) → 
      (Externaddr-ok s (externaddr-FUNC v-funcaddr) (FUNC (mk-functype (mk-list t-1-lst) (mk-list t-2-lst)))) →
      Instr-ok2 s C (CALL-ADDR v-funcaddr) (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))
    Instr-ok2--ref : ∀ (s : store) (C : context) (v-ref : ref) (rt : reftype) → 
      (Ref-ok s v-ref rt) →
      Instr-ok2 s C (admininstr-ref v-ref) (mk-functype (mk-list []) (mk-list ((valtype-reftype rt) ∷ [])))
    Instr-ok2--trap : ∀ (s : store) (C : context) (t-1-lst : (List valtype)) (t-2-lst : (List valtype)) → Instr-ok2 s C admininstr-TRAP (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))

  {- Inductive Relations Definition at: ../specification/wasm-2.0/B-soundness.spectec:70.1-71.36 -}
  data Instrs-ok2 : store → context → (List admininstr) → functype → Set where
    Instrs-ok2--empty : ∀ (s : store) (C : context) → Instrs-ok2 s C [] (mk-functype (mk-list []) (mk-list []))
    Instrs-ok2--instr : ∀ (s : store) (C : context) (v-admininstr : admininstr) (t-1-lst : (List valtype)) (t-2-lst : (List valtype)) → 
      (Instr-ok2 s C v-admininstr (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))) →
      Instrs-ok2 s C (v-admininstr ∷ []) (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))
    Instrs-ok2--seq : ∀ (s : store) (C : context) (admininstr-1-lst : (List admininstr)) (admininstr-2-lst : (List admininstr)) (t-1-lst : (List valtype)) (t-3-lst : (List valtype)) (t-2-lst : (List valtype)) → 
      (Instrs-ok2 s C admininstr-1-lst (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))) →
      (Instrs-ok2 s C admininstr-2-lst (mk-functype (mk-list t-2-lst) (mk-list t-3-lst))) →
      Instrs-ok2 s C (admininstr-1-lst ++ admininstr-2-lst) (mk-functype (mk-list t-1-lst) (mk-list t-3-lst))
    Instrs-ok2--sub : ∀ (s : store) (C : context) (admininstr-lst : (List admininstr)) (t'-1-lst : (List valtype)) (t'-2-lst : (List valtype)) (t-1-lst : (List valtype)) (t-2-lst : (List valtype)) → 
      (Instrs-ok2 s C admininstr-lst (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))) →
      (Resulttype-sub (mk-list t'-1-lst) (mk-list t-1-lst)) →
      (Resulttype-sub (mk-list t-2-lst) (mk-list t'-2-lst)) →
      Instrs-ok2 s C admininstr-lst (mk-functype (mk-list t'-1-lst) (mk-list t'-2-lst))
    Instrs-ok2--frame : ∀ (s : store) (C : context) (admininstr-lst : (List admininstr)) (t-lst : (List valtype)) (t-1-lst : (List valtype)) (t-2-lst : (List valtype)) → 
      (Instrs-ok2 s C admininstr-lst (mk-functype (mk-list t-1-lst) (mk-list t-2-lst))) →
      Instrs-ok2 s C admininstr-lst (mk-functype (mk-list (t-lst ++ t-1-lst)) (mk-list (t-lst ++ t-2-lst)))

  {- Inductive Relations Definition at: ../specification/wasm-2.0/B-soundness.spectec:72.1-73.36 -}
  data Expr-ok2 : store → context → adminexpr → resulttype → Set where
    mk-Expr-ok2 : ∀ (s : store) (C : context) (admininstr-lst : (List admininstr)) (t-lst : (List valtype)) → 
      (Instrs-ok2 s C admininstr-lst (mk-functype (mk-list []) (mk-list t-lst))) →
      Expr-ok2 s C admininstr-lst (mk-list t-lst)
...
data Globalinst-ok : store → globalinst → globaltype → Set where
  mk-Globalinst-ok : ∀ (s : store) (v-mut : mut) (t : valtype) (v-val : val) → 
    (Globaltype-ok (mk-globaltype v-mut t)) →
    (Val-ok s v-val t) →
    Globalinst-ok s record { globalinst-TYPE = (mk-globaltype v-mut t) ; VALUE = v-val } (mk-globaltype v-mut t)
...
data Meminst-ok : store → meminst → memtype → Set where
  mk-Meminst-ok : ∀ (s : store) (v-n : n) (m-opt : (Maybe m)) (b-lst : (List byte)) → 
    (Memtype-ok (PAGE (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt)))) →
    ((length b-lst) ≡ (v-n * (64 * (Ki )))) →
    Meminst-ok s record { meminst-TYPE = (PAGE (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt))) ; BYTES = b-lst } (PAGE (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt)))
...
data Tableinst-ok : store → tableinst → tabletype → Set where
  mk-Tableinst-ok : ∀ (s : store) (v-n : n) (m-opt : (Maybe m)) (rt : reftype) (ref-lst : (List ref)) → 
    (Tabletype-ok (mk-tabletype (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt)) rt)) →
    Forall (λ (v-ref : ref) → (Ref-ok s v-ref rt)) ref-lst →
    ((length ref-lst) ≡ v-n) →
    Tableinst-ok s record { tableinst-TYPE = (mk-tabletype (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt)) rt) ; REFS = ref-lst } (mk-tabletype (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt)) rt)

{- Inductive Relations Definition at: ../specification/wasm-2.0/B-soundness.spectec:157.1-157.51 -}
data Funcinst-ok : store → funcinst → functype → Set where
  mk-Funcinst-ok : ∀ (s : store) (ft : functype) (v-moduleinst : moduleinst) (v-func : func) (C : context) → 
    (Functype-ok ft) →
    (Moduleinst-ok s v-moduleinst C) →
    (Func-ok C v-func ft) →
    Funcinst-ok s record { funcinst-TYPE = ft ; funcinst-MODULE = v-moduleinst ; CODE = v-func } ft

{- Inductive Relations Definition at: ../specification/wasm-2.0/B-soundness.spectec:232.1-232.33 -}
data Store-ok : store → Set where
  mk-Store-ok : ∀ (s : store) (globalinst-lst : (List globalinst)) (globaltype-lst : (List globaltype)) (meminst-lst : (List meminst)) (memtype-lst : (List memtype)) (tableinst-lst : (List tableinst)) (tabletype-lst : (List tabletype)) (funcinst-lst : (List funcinst)) (functype-lst : (List functype)) (datainst-lst : (List datainst)) (datatype-lst : (List datatype)) (eleminst-lst : (List eleminst)) (elemtype-lst : (List elemtype)) → 
    ...
    ((length funcinst-lst) ≡ (length functype-lst)) →
    Forall₂ (λ (v-funcinst : funcinst) (v-functype : functype) → (Funcinst-ok s v-funcinst v-functype)) funcinst-lst functype-lst →
    ...
    (s ≡ record { store-FUNCS = funcinst-lst ; store-GLOBALS = globalinst-lst ; store-TABLES = tableinst-lst ; store-MEMS = meminst-lst ; store-ELEMS = eleminst-lst ; store-DATAS = datainst-lst }) →
    Store-ok s
...
data Extend-globalinst : globalinst → globalinst → Set where
  mk-Extend-globalinst : ∀ (v-mut : mut) (t : valtype) (v-val : val) (val' : val) → 
    ((v-mut ≡ (just MUT)) ⊎ (v-val ≡ val')) →
    Extend-globalinst record { globalinst-TYPE = (mk-globaltype v-mut t) ; VALUE = v-val } record { globalinst-TYPE = (mk-globaltype v-mut t) ; VALUE = val' }
...
data Extend-meminst : meminst → meminst → Set where
  mk-Extend-meminst : ∀ (v-n : n) (m-opt : (Maybe m)) (b-lst : (List byte)) (n' : n) (b'-lst : (List byte)) → 
    (v-n ≤ n') →
    ((length b-lst) ≤ (length b'-lst)) →
    Extend-meminst record { meminst-TYPE = (PAGE (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt))) ; BYTES = b-lst } record { meminst-TYPE = (PAGE (mk-limits (mk-uN n') (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt))) ; BYTES = b'-lst }
...
data Extend-tableinst : tableinst → tableinst → Set where
  mk-Extend-tableinst : ∀ (v-n : n) (m-opt : (Maybe m)) (rt : reftype) (ref-lst : (List ref)) (n' : n) (ref'-lst : (List ref)) → 
    (v-n ≤ n') →
    ((length ref-lst) ≤ (length ref'-lst)) →
    Extend-tableinst record { tableinst-TYPE = (mk-tabletype (mk-limits (mk-uN v-n) (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt)) rt) ; REFS = ref-lst } record { tableinst-TYPE = (mk-tabletype (mk-limits (mk-uN n') (mapMaybe (λ (v-m : ℕ) → (mk-uN v-m)) m-opt)) rt) ; REFS = ref'-lst }
...
data Extend-funcinst : funcinst → funcinst → Set where
  mk-Extend-funcinst : ∀ (ft : functype) (mm : moduleinst) (fc : func) → Extend-funcinst record { funcinst-TYPE = ft ; funcinst-MODULE = mm ; CODE = fc } record { funcinst-TYPE = ft ; funcinst-MODULE = mm ; CODE = fc }
...
data Config-ok : config → resulttype → Set where
  mk-Config-ok : ∀ (s : store) (f : frame) (admininstr-lst : (List admininstr)) (t-lst : (List valtype)) (C : context) → 
    (State-ok (mk-state s f) C) →
    (Expr-ok2 s C admininstr-lst (mk-list t-lst)) →
    Config-ok (mk-config (mk-state s f) admininstr-lst) (mk-list t-lst)
