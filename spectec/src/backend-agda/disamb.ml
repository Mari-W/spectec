...
let error at msg = Util.Error.error at "Agda Generation" msg
...
let t_atom typ_id a =
  ...
  | Some a -> a
  (* unregistered (typ, atom) pairs when use-site type differs from def, keep atom, rely on ctor overloading *)
  | None -> a
...
let t_exp e = 
  match e.it with
  | CaseE (m, e') ->
    let name = Il.Print.string_of_typ_name (reduce_typ_safe !env_ref.il_env e.note) in
...
let t_path p = 
  match p.it with
  | DotP (p', a) ->
    let name = Il.Print.string_of_typ_name (reduce_typ_safe !env_ref.il_env p'.note) in
...
let rec t_atom_new typ_id base_a a =
  ...
  | Atom.Atom base_s, Atom.Atom s ->
    (* sibling family instances become own datatypes, reused name overloads ctor, agda resolves by expected type *)
    begin match StringMap.find_opt typ_id !env_ref.disamb_map with
    | Some ss when List.mem_assoc base_s ss ->
      {a with it = Atom.Atom (List.assoc base_s ss)}
    | _ ->
      let lst = StringMap.bindings !env_ref.disamb_map in
...
let remove_overlapping_clauses clauses = 
  ...
  | DefD (_, args, exp, _), DefD (_, args', exp', _) -> 
    (* syntactic compare, reducing bodies can diverge on recursive fns (utf8), first-match makes dups harmless *)
    Eq.eq_list Eq.eq_arg args args' && Eq.eq_exp exp exp'
  ) clauses
